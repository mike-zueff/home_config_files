function _rc_transliterate_en_to_ru
{
  ...
  TABLE_EN_L+=(a) ; TABLE_RU_L+=(а)
  TABLE_EN_L+=(b) ; TABLE_RU_L+=(б)
  TABLE_EN_L+=(d) ; TABLE_RU_L+=(д)
  TABLE_EN_L+=(f) ; TABLE_RU_L+=(ф)
  TABLE_EN_L+=(g) ; TABLE_RU_L+=(г)
  TABLE_EN_L+=(k) ; TABLE_RU_L+=(к)
  TABLE_EN_L+=(l) ; TABLE_RU_L+=(л)
  TABLE_EN_L+=(m) ; TABLE_RU_L+=(м)
  TABLE_EN_L+=(n) ; TABLE_RU_L+=(н)
  TABLE_EN_L+=(o) ; TABLE_RU_L+=(о)
  TABLE_EN_L+=(p) ; TABLE_RU_L+=(п)
  TABLE_EN_L+=(r) ; TABLE_RU_L+=(р)
  TABLE_EN_L+=(s) ; TABLE_RU_L+=(с)
  TABLE_EN_L+=(t) ; TABLE_RU_L+=(т)
  TABLE_EN_L+=(u) ; TABLE_RU_L+=(у)
  TABLE_EN_L+=(v) ; TABLE_RU_L+=(в)
  TABLE_EN_L+=(y) ; TABLE_RU_L+=(ы)
  TABLE_EN_L+=(z) ; TABLE_RU_L+=(з)

  for (( I = 0; I < ${#TABLE_EN_L[@]}; ++I ))
  do
  ...
  do
    ...
    for (( J = 0; J < ${#TABLE_EN[@]}; ++J ))
    do
      if [[ ${TABLE_EN[${J}]} == "${CURRENT_CHARACTER}" ]]
      then
        CURRENT_CHARACTER=${TABLE_RU[${J}]}
        break
      fi
    done
  ...
  echo "${RESULT}"
  echo "${1}" | grep -i "e" && WARNING+="| e = еёэ "
  echo "${1}" | grep -i "i" && WARNING+="| i = ий "
  echo "${1}" | grep -i "ch" && WARNING+="| ch = ч "
  echo "${1}" | grep -i "ia" && WARNING+="| ia = я "
  echo "${1}" | grep -i "ie" && WARNING+="| ie = ъ "
  echo "${1}" | grep -i "iu" && WARNING+="| iu = ю "
  echo "${1}" | grep -i "kh" && WARNING+="| kh = х "
  echo "${1}" | grep -i "sh" && WARNING+="| sh = ш "
  echo "${1}" | grep -i "ts" && WARNING+="| ts = ц "
  echo "${1}" | grep -i "zh" && WARNING+="| zh = ж "
  echo "${1}" | grep -i "shch" && WARNING+="| shch = щ "
  echo "${WARNING}"
  echo "Can also contain several \"Ь\" or \"ь\" characters."
}
